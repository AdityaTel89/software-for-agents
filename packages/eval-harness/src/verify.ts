  ...
  try {
    switch (handlerName) {
      case 'verify_page_creation': {
        const databaseId = (process.env.NOTION_TEST_DATABASE_ID || params.database_id) as
          | string
          | undefined;
        ...
        const response = await notionRequest<NotionListResponse<NotionPage>>(
      ...
      case 'verify_block_append': {
        ...
        // Fetch blocks
        const blocksResponse = await notionRequest<NotionListResponse<NotionBlock>>(

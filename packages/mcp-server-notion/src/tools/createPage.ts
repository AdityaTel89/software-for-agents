...
const createPageSchema = z.object({
  parent: z.object({
    database_id: z.string().optional().describe("The UUID of the parent database (if creating a database item)"),
    page_id: z.string().optional().describe("The UUID of the parent page (if creating a sub-page)")
  ...
  properties: z.record(z.unknown()).describe("The properties of the page. If parent is a database, these must match the database's schema (e.g. Name, Status)."),
  children: z.array(z.unknown()).optional().describe("Optional list of block objects to populate as the page's body content")
});

export const createPageTool: MCPTool<typeof createPageSchema> = {
  name: "create_page",
  description: "Create a new page under a parent page or inside a database. Use this when the user wants to add a new document, sub-page, log, task, or database entry. If creating inside a database, 'properties' is required and must match the database schema names/types exactly. Returns the created page ID, title, and URL. Pitfalls: Property structures are complex; use 'get_database' first to find the schema of properties if you are unsure.",

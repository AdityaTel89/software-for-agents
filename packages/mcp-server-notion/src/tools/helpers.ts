...
export function zodToMcpSchema(zodSchema: z.ZodObject<z.ZodRawShape>): {
  ...
  for (const key of Object.keys(shape)) {
    ...
    let typeName: string | undefined = 'string';
    ...
    } else if (currentField instanceof z.ZodUnion) {
      const options = currentField._def.options as Array<z.ZodTypeAny>;
      extraProperties.anyOf = options.map((opt: z.ZodTypeAny) => {
        if (opt instanceof z.ZodObject) {
          const subSchema = zodToMcpSchema(opt);
          return {
            type: 'object',
            properties: subSchema.properties,
            required: subSchema.required,

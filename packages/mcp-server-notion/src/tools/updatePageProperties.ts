...
const updatePagePropertiesSchema = z.object({
  ...
  properties: z.record(z.unknown()).describe("Key-value map of properties to update. The keys must match the property names in the parent database, and values must match Notion property formats.")
});

export const updatePagePropertiesTool: MCPTool<typeof updatePagePropertiesSchema> = {
  name: "update_page_properties",
  description: "Updates the property values of a Notion page (e.g., status, select option, tags, dates, text fields). Use this when the user asks to edit metadata on a page or modify database record columns. Returns the page UUID and updated title. Pitfalls: Does not update body content (blocks) — only updates page headers/database properties. Property shapes vary; use get_page first to check current property formats.",

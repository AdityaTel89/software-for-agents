...
const searchSchema = z.object({
  query: z.string().optional().describe("The search term to match against page or database titles, e.g. 'Project Alpha'"),
  filter: z.object({
    value: z.enum(["page", "database"]).describe("The type of object to retrieve"),
    property: z.literal("object").describe("Must be 'object'")
  }).optional().describe("Filter results to only 'page' or 'database'"),
  page_size: z.number().int().min(1).max(100).optional().describe("Number of items to retrieve (default 50, max 100)"),
